...
const ProficiencyGroupListWrapper = () => {
  const [deleteId, setDeleteId] = useState<ID>();
  const [showLoading, setShowLoading] = useState(false);
  const [showDeleteDialog, setShowDeleteDialog] = useState(false);

  const columns: Column<ProficiencyGroup>[] = [
    {
      Header: 'Nome',
      accessor: 'name',
    },
  ...
  const {
    ...
    isLoading,
    refetch,
  }: UseQueryResult<PaginatedResponse<ProficiencyGroup>> = useQuery(
    ['proficiency-group-list', page, sortBy, sortOrder, filter, search],
...
export { ProficiencyGroupListWrapper };